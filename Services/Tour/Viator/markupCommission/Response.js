...
const response = {
    getMarkupDbApiReponse : async(clientId, agentGuid, token, currency, request, providerDetails)=>{
        ...
        let fPath = await apiCommonController.logsFolderStructureBuilder(clientId, config.SERVICE_MARKUP_PROVIDER_RESPONSE_FILE_PATH, config.Provider_Code_VTR);
        try{
            return new Promise((resolve) => {
                // Defined headder, it is common for all apis.
                // Let URL = config.Agent_Markup_URL;
                ...
                .then((data) => {
                    ...
                    createLogs(`${providerDetails.MoonstrideConfiguration.agentmarkupurl+agentGuid}?currency=${currency} ,//, ${providerDetails?.MoonstrideConfiguration?.agentcommissionurl+agentGuid}?currency=${currency}`, fileName, fPath, request, data)
                    resolve(data);
                })
                .catch((error) => {
                    let errorObj = {
